...
class BaseProblem(ABC):
    n_obj_sample:   int = NotImplemented
    n_constrs:      int = NotImplemented
    name:           str = NotImplemented
    net                 = NotImplemented
    n_parameters:   int = NotImplemented
    ...
    def objective_func_and_grad(self, optimizer, no_grad = False, return_multiple_f = False):
        ...
        if return_multiple_f:
            f, f_interior_data, f_boundary_data = self.objective_func(return_multiple_f=return_multiple_f)
        else:
            f = self.objective_func(return_multiple_f=return_multiple_f)
        f_value = f.data
        
        if no_grad is True:
            if return_multiple_f:
                return f_value, f_interior_data, f_boundary_data
            else:
                return f_value
        else:
            ...
            if return_multiple_f:
                return f_value, f_interior_data, f_boundary_data, g_value
            else:
